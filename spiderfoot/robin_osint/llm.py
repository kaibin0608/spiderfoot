...
from spiderfoot.robin_osint.llm_utils import _common_llm_params, resolve_model_config, get_model_choices
...
def filter_results(llm, query, results):
    ...
    try:
        ...
        logging.warning("LLM filter_results failed (%s). Truncating and retrying.", e)
    ...
    if not parsed_indices:
        logging.warning(
            "Unable to interpret LLM result selection ('%s'). Defaulting to top %s results.",
            result_indices,
            min(len(results), 20),
        )
...
def _generate_final_string(results, truncate=False):
    ...
    for i, res in enumerate(results):
        ...
        if truncate and max_title_length:
